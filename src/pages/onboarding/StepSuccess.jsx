  ...
  return (
    <div style={{ position:'relative' }}>
        ...
        <div style={{ position:'fixed',top:0,left:0,right:0,bottom:0,pointerEvents:'none',zIndex:999,overflow:'hidden' }}>
          {Array.from({length:36}).map((_,i) => (
            <div key={i} style={{
              ...
              background:['#0040a0','#0ea5e9','#f59e0b','#ec4899','#8b5cf6','#10b981'][Math.floor(Math.random()*6)],
      ...
      <div style={{ textAlign:'center', marginBottom:28, animation:'fadeInUp 0.5s ease both' }}>
        <div style={{ width:88,height:88,borderRadius:'50%',margin:'0 auto 20px',background:'linear-gradient(135deg,#0040a0,#0ea5e9)',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 0 50px rgba(0,64,160,0.35)' }}>
        ...
        <p style={{ color:'#64748b',fontSize:16,lineHeight:1.6 }}>
          <strong style={{ color:'#0040a0' }}>{schoolName}</strong> is now live on School CloudX.
      ...
      {/* Trial banner */}
      {trial?.active && (
        <div style={{ background:'linear-gradient(135deg,#e8f0fc,#ccdaf5)',borderRadius:16,border:'1.5px solid rgba(0,64,160,0.25)',padding:'18px 22px',marginBottom:20,animation:'fadeInUp 0.5s ease 0.1s both' }}>
          <div style={{ display:'flex',alignItems:'center',gap:12,marginBottom:12 }}>
            <div style={{ width:40,height:40,borderRadius:12,background:'linear-gradient(135deg,#0040a0,#0ea5e9)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
            ...
            <div style={{ marginLeft:'auto',fontSize:12,fontWeight:700,padding:'4px 12px',borderRadius:20,background:'#0040a0',color:'#fff' }}>FREE</div>
          </div>
          <div style={{ display:'flex',gap:12,flexWrap:'wrap' }}>
            {[
              ...
              <div key={i} style={{ display:'flex',alignItems:'center',gap:6,background:'#fff',padding:'6px 14px',borderRadius:8,border:'1px solid rgba(0,64,160,0.2)',fontSize:12,color:'#374151',fontWeight:500 }}>
                <Icon size={13} color="#0040a0" /> {text}
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ background:'#f0fdf9',borderRadius:12,border:'1.5px solid rgba(0,64,160,0.2)',padding:'12px 18px',marginBottom:20,display:'flex',alignItems:'center',gap:16,flexWrap:'wrap',animation:'fadeInUp 0.5s ease 0.12s both' }}>
        <div style={{ display:'flex',alignItems:'center',gap:6,fontSize:13,color:'#0040a0',fontWeight:600 }}><Mail size={14} /> Credentials sent via email</div>
        <div style={{ width:1,height:16,background:'#ccdaf5' }} />
        <div style={{ display:'flex',alignItems:'center',gap:6,fontSize:13,color:'#0040a0',fontWeight:600 }}><MessageCircle size={14} /> Sent via WhatsApp</div>
      </div>

      {/* Credentials card */}
      <div style={{ background:'#fff',borderRadius:20,border:'1.5px solid #e8f0fc',padding:24,marginBottom:20,boxShadow:'0 4px 24px rgba(0,64,160,0.08)',animation:'fadeInUp 0.5s ease 0.15s both' }}>
        <div style={{ fontSize:12,fontWeight:700,color:'#0040a0',letterSpacing:1,textTransform:'uppercase',marginBottom:16,display:'flex',alignItems:'center',gap:6 }}>
        ...
        {[
          ...
          <div key={label} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:'1px solid #f0fdf9' }}>
            ...
            <div style={{ display:'flex',alignItems:'center',gap:8,maxWidth:'60%' }}>
              <span style={{ fontSize:13,fontWeight:500,wordBreak:'break-all',textAlign:'right',color:key==='url'?'#0040a0':'#374151',fontFamily:key!=='url'?'monospace':'inherit' }}>{value}</span>
              <button onClick={()=>copyText(value,key)} style={{ background:'none',border:'none',cursor:'pointer',padding:4,flexShrink:0,color:copied===key?'#0040a0':'#d1d5db',transition:'color 0.2s' }}>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:6,marginTop:12 }}>
          <ShieldCheck size={12} color="#0040a0" />
      ...
      <a href={loginPageUrl} target="_blank" rel="noopener noreferrer"
        style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:8,width:'100%',height:52,background:'linear-gradient(135deg,#0040a0,#002f80)',color:'#fff',borderRadius:12,textDecoration:'none',fontSize:15,fontWeight:700,boxShadow:'0 6px 20px rgba(0,64,160,0.35)',marginBottom:16,animation:'fadeInUp 0.5s ease 0.2s both',transition:'all 0.2s' }}
      ...
      {/* Next steps */}
      <div style={{ background:'#fff',borderRadius:20,border:'1.5px solid #e8f0fc',padding:24,marginBottom:20,boxShadow:'0 4px 24px rgba(0,64,160,0.06)',animation:'fadeInUp 0.5s ease 0.25s both' }}>
        <div style={{ fontSize:12,fontWeight:700,color:'#0040a0',letterSpacing:1,textTransform:'uppercase',marginBottom:16 }}>What's Next</div>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12 }}>
          {NEXT_STEPS.map(({icon:Icon,title,desc})=>(
            <div key={title} style={{ display:'flex',gap:10,padding:'12px',borderRadius:12,background:'#f8fffe',border:'1.5px solid #e8f0fc' }}>
              <div style={{ width:36,height:36,borderRadius:10,background:'rgba(0,64,160,0.1)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
                <Icon size={16} color="#0040a0" />
              </div>
              <div>
      ...
      <p style={{ textAlign:'center',marginTop:16,fontSize:12,color:'#94a3b8' }}>
        Need help? <a href="mailto:support@schoolcloudx.com" style={{ color:'#0040a0',textDecoration:'none',fontWeight:600 }}>support@schoolcloudx.com</a>
      </p>
