    ...
    <div>
      <div style={{ textAlign:'center', marginBottom:32 }}>
        ...
        <div style={{ display:'inline-flex',alignItems:'center',gap:7,background:'rgba(0,64,160,0.08)',border:'1px solid rgba(0,64,160,0.2)',borderRadius:50,padding:'5px 14px',marginBottom:14 }}>
          ...
          <span style={{ fontSize:11.5,fontWeight:700,color:'#0040a0',letterSpacing:'0.06em' }}>STEP 2 OF 3</span>
        </div>
        ...
        <p style={{ color:'#64748b',fontSize:15 }}>
          {mode==='email'
            ? <>We sent a 6-digit OTP to <strong style={{ color:'#0040a0' }}>{maskedEmail}</strong></>
            : <>Enter OTP for <strong style={{ color:'#0040a0' }}>{maskedPhone}</strong></>}
        </p>
      </div>
      ...
      <div style={{ background:'#fff',borderRadius:20,border:'1.5px solid #e8f0fc',padding:32,boxShadow:'0 4px 24px rgba(0,64,160,0.08)' }}>
        ...
        <div style={{ display:'flex',background:'#f0fdf9',borderRadius:12,padding:4,marginBottom:28,gap:4 }}>
          {[{key:'email',label:'Email OTP',Icon:Mail},{key:'phone',label:'Phone OTP',Icon:Phone}].map(({key,label,Icon})=>(
            <button key={key} onClick={()=>setMode(key)} style={{
              ...
              background:mode===key?'linear-gradient(135deg,#0040a0,#002f80)':'transparent',
              color:mode===key?'#fff':'#64748b',
              boxShadow:mode===key?'0 4px 14px rgba(0,64,160,0.3)':'none',
          ...
          {otp.map((d,i)=>(
            <input key={i} ref={el=>refs.current[i]=el} type="text" inputMode="numeric" maxLength={1} value={d}
              ...
              style={{
                ...
                background: d ? '#e8f0fc' : '#f8fffe',
                border:`1.5px solid ${d?'#0040a0':'#e2e8f0'}`,
                borderRadius:12,color:'#1a1a2e',transition:'all 0.18s',outline:'none',
                boxShadow: d ? '0 0 0 3px rgba(0,64,160,0.1)' : 'none',
              }}
              onFocus={e=>{e.target.style.borderColor='#0040a0';e.target.style.boxShadow='0 0 0 3px rgba(0,64,160,0.12)'}}
        ...
        <div style={{ textAlign:'center',marginBottom:24 }}>
          {canResend ? (
            <button onClick={resend} disabled={resending} style={{ background:'none',border:'none',color:'#0040a0',cursor:'pointer',fontSize:14,display:'inline-flex',alignItems:'center',gap:6,fontWeight:600,fontFamily:'inherit' }}>
        ...
        <button onClick={verify} disabled={loading||!filled}
          style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:8,width:'100%',height:52,background:'linear-gradient(135deg,#0040a0,#002f80)',color:'#fff',border:'none',borderRadius:12,fontSize:15,fontWeight:700,cursor:'pointer',boxShadow:'0 6px 20px rgba(0,64,160,0.35)',transition:'all 0.2s',opacity:(!filled||loading)?0.6:1,fontFamily:'inherit',marginBottom:12 }}
        ...
        <div style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:6,marginTop:20,color:'#94a3b8',fontSize:12 }}>
          <ShieldCheck size={13} color="#0040a0" /> Secured with 256-bit SSL encryption
        </div>
      </div>
    </div>
  )
