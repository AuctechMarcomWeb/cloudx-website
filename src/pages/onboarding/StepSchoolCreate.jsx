...
const ICON_STYLE = {
  ...
  background:'rgba(27,188,155,0.06)',borderRight:'1px solid #e8faf6',
  color:'#1bbc9b',flexShrink:0,
}
const INPUT_STYLE = {
...
export default function StepSchoolCreate({ registrationId, onNext, onBack }) {
  ...
  const sectionTitle = { display:'flex',alignItems:'center',gap:6,fontSize:11,fontWeight:700,color:'#1bbc9b',letterSpacing:'0.06em',textTransform:'uppercase',marginBottom:14,paddingBottom:8,borderBottom:'1px solid rgba(27,188,155,0.12)' }
    ...
    <div>
      <div style={{ textAlign:'center', marginBottom:32 }}>
        ...
        <div style={{ display:'inline-flex',alignItems:'center',gap:7,background:'rgba(27,188,155,0.08)',border:'1px solid rgba(27,188,155,0.2)',borderRadius:50,padding:'5px 14px',marginBottom:14 }}>
          ...
          <span style={{ fontSize:11.5,fontWeight:700,color:'#1bbc9b',letterSpacing:'0.06em' }}>STEP 3 OF 3</span>
        </div>
        <h2 style={{ fontSize:30,fontWeight:800,marginBottom:8,color:'#1a1a2e',fontFamily:"'Poppins',sans-serif" }}>School Details</h2>
      ...
      <div style={{ background:'#fff',borderRadius:20,border:'1.5px solid #e8faf6',padding:32,boxShadow:'0 4px 24px rgba(27,188,155,0.08)' }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom:24 }}>
            ...
            <div style={{ display:'flex',alignItems:'center',gap:16,background:'#f8fffe',borderRadius:14,border:'1.5px solid #e8faf6',padding:16 }}>
              <div onClick={() => !uploading && fileRef.current?.click()} style={{
                width:80,height:80,borderRadius:14,flexShrink:0,
                border:`2px dashed ${uploading?'#f59e0b':logoPreview?'#1bbc9b':'#cbd5e0'}`,
                display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
                overflow:'hidden',background:logoPreview?'#f0fdf9':'rgba(27,188,155,0.03)',
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ display:'flex',gap:8 }}>
                  <button type="button" onClick={() => !uploading && fileRef.current?.click()} disabled={uploading}
                    style={{ padding:'5px 14px',fontSize:12,fontWeight:600,background:'rgba(27,188,155,0.1)',border:'1px solid rgba(27,188,155,0.25)',borderRadius:8,cursor:'pointer',color:'#1bbc9b',display:'flex',alignItems:'center',gap:4,fontFamily:'inherit' }}>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={FIELD_STYLE} onFocusCapture={e=>e.currentTarget.style.borderColor='#1bbc9b'} onBlurCapture={e=>e.currentTarget.style.borderColor='#e2e8f0'}>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ ...FIELD_STYLE,alignItems:'flex-start' }} onFocusCapture={e=>e.currentTarget.style.borderColor='#1bbc9b'} onBlurCapture={e=>e.currentTarget.style.borderColor='#e2e8f0'}>
            ...
            {[['city','City'],['state','State'],['pincode','Pincode']].map(([n,lbl])=>(
              <div key={n}>
                ...
                <input name={n} value={form[n]} onChange={handle} placeholder={lbl}
                  ...
                  onFocus={e=>e.target.style.borderColor='#1bbc9b'} onBlur={e=>e.target.style.borderColor='#e2e8f0'} />
              </div>
            ))}
          </div>
          ...
          <button type="submit" disabled={loading}
            style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:8,width:'100%',height:52,background:'linear-gradient(135deg,#1bbc9b,#0e9f82)',color:'#fff',border:'none',borderRadius:12,fontSize:15,fontWeight:700,cursor:'pointer',boxShadow:'0 6px 20px rgba(27,188,155,0.35)',transition:'all 0.2s',opacity:loading?0.7:1,fontFamily:'inherit',marginBottom:12 }}
